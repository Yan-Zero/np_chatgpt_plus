...
from .ban import BAN, COUNT_LIMIT
...
ban = nonebot.on_command("ban", priority=10, block=True, permission=GPTOWNER)
...
async def handle_ban(bot: Bot, event: MessageEvent, args: MessageChain = CommandArg()):
    ...
    await ban.finish(f"已封禁: {', '.join(lists)}")
